...
from lib.graphic.ray_casting import mapping
...
class Sprites:
    def __init__(self):
        self.sprite_parameters = {
            'sprite_barrel': {
                'sprite': pygame.image.load('resources/sprites/barrel/base/0.png').convert_alpha(),
                'viewing_angles': None,
                'shift': 1.8,
                'scale': (0.4, 0.4),
                'side': 30,
                'animation': deque(
                    [pygame.image.load(f'resources/sprites/barrel/anim/{i}.png').convert_alpha() for i in range(12)]),
                'death_animation': deque([pygame.image.load(f'resources/sprites/barrel/death/{i}.png')
            ...
            'sprite_pin': {
                'sprite': pygame.image.load('resources/sprites/pin/base/0.png').convert_alpha(),
                'viewing_angles': None,
                'shift': 0.6,
                'scale': (0.6, 0.6),
                'side': 30,
                'animation': deque([pygame.image.load(f'resources/sprites/pin/anim/{i}.png').convert_alpha() for i in range(8)]),
                'death_animation': [],
                'is_dead': 'immortal',
                'dead_shift': None,
                'animation_dist': 800,
            ...
            'sprite_flame': {
                'sprite': pygame.image.load('resources/sprites/flame/base/0.png').convert_alpha(),
                'viewing_angles': None,
                'shift': 0.7,
                'scale': (0.6, 0.6),
                'side': 30,
                'animation': deque(
                    [pygame.image.load(f'resources/sprites/flame/anim/{i}.png').convert_alpha() for i in range(16)]),
                'death_animation': [],
                'is_dead': 'immortal',
                'dead_shift': 1.8,
                'animation_dist': 1800,
            ...
            'npc_devil': {
                'sprite': [pygame.image.load(f'resources/sprites/npc/devil/base/{i}.png').convert_alpha() for i in range(8)],
                'viewing_angles': True,
                'shift': 0.0,
                'scale': (1.1, 1.1),
                'side': 50,
                'animation': [],
                'death_animation': deque([pygame.image.load(f'resources/sprites/npc/devil/death/{i}.png')
                ...
                'flag': 'npc',
                'obj_action': deque(
                    [pygame.image.load(f'resources/sprites/npc/devil/anim/{i}.png').convert_alpha() for i in range(9)]),
            },
            'sprite_door_v': {
                'sprite': [pygame.image.load(f'resources/doors/door_v/{i}.png').convert_alpha() for i in range(16)],
                'viewing_angles': True,
                'shift': 0.1,
                'scale': (2.6, 1.2),
                'side': 100,
            ...
            'sprite_door_h': {
                'sprite': [pygame.image.load(f'resources/doors/door_h/{i}.png').convert_alpha() for i in range(16)],
                'viewing_angles': True,
                'shift': 0.1,
                'scale': (2.6, 1.2),
                'side': 100,
            ...
            'npc_soldier0': {
                'sprite': [pygame.image.load(f'resources/sprites/npc/soldier/base/{i}.png').convert_alpha() for i in range(8)],
                'viewing_angles': True,
                'shift': 0.8,
                'scale': (0.4, 0.6),
                'side': 30,
                'animation': [],
                'death_animation': deque([pygame.image.load(f'resources/sprites/npc/soldier/death1/{i}.png')
                ...
                'blocked': True,
                'flag': 'npc',
                'obj_action': deque([pygame.image.load(f'resources/sprites/npc/soldier/action/{i}.png')
                                    .convert_alpha() for i in range(4)])
            },
        }
    ...
    def blocked_doors(self):
        blocked_doors = Dict.empty(
            key_type=types.UniTuple(int32, 2), value_type=int32)
...
class SpriteObject:
        ...
        if self.viewing_angles:
            if len(self.object) == 8:
                self.sprite_angles = [frozenset(range(338, 361)) | frozenset(range(0, 23))] + \
                                     [frozenset(range(i, i + 45))
                                      for i in range(23, 338, 45)]
            else:
                self.sprite_angles = [frozenset(range(348, 361)) | frozenset(range(0, 11))] + \
                                     [frozenset(range(i, i + 23))
                                      for i in range(11, 348, 23)]
    ...
    def object_locate(self, player):
        ...
        if self.flag not in {'door_h', 'door_v'}:
            self.distance_to_sprite *= math.cos(HALF_FOV -
                                                self.current_ray * DELTA_ANGLE)
